...
import main.studysession.StudySessionController;
import models.Card;
import models.Deck;
...
public class PlayerController {
    ...
    public void toggleStudySession(ActionEvent event) throws IOException {
        System.out.println("Study Session begin!");
        FXMLLoader loader = new FXMLLoader(getClass().getResource("/resources/fxml/studysession.fxml"));
        try {
            root = loader.load();
            StudySessionController controller = loader.getController();
            controller.initStudySession(deck);
            stage = (Stage) ((Node) event.getSource()).getScene().getWindow();
            scene = new Scene(root);
            stage.setScene(scene);
            new SlideInRight(root).play();
            ...
            System.out.println("Error loading studysession.fxml");
            e.printStackTrace();
        }
    }
}
